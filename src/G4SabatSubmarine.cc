...
DetectorTube::DetectorTube( SubmarineBody *sub)
{
    parent = sub;
    qDebug() << "[Constructor]{DetectorTube}" << "done";
}
...
{
    return height / 2 + parent->width / 2;
}
...
void G4SabatSubmarine::setVolumePossition(const G4ThreeVector &possition, const G4double seaBottomHeight)
{
    G4double x = possition.x(),
            y = possition.y() + seaBottomHeight / 2 + height / 2 + sourceTube->height / 2,
            z = possition.z();
                          ...
                          const G4bool &checkOverlaps)
{
     ...
     // Submarine main box
     G4Material* submarine_mat = nist->FindOrBuildMaterial("G4_A-150_TISSUE");
     ...
     //Source Tube
     G4Material* sourceTube_mat = nist->FindOrBuildMaterial("G4_A-150_TISSUE");
     G4Box *solidSourceTube =
             new G4Box("SourceTube", sourceTube->x(), sourceTube->y(), sourceTube->z());
